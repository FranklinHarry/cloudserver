...
function multiObjectDelete(authInfo, request, log, callback) {
    ...
    return async.waterfall([
        ...
        function checkPolicies(quietSetting, objects, next) {
            ...
            log.error('DEBUGINFO 1', {
                requestContextParams,
            });
            log.error('DEBUGINFO 1.1', {
                objects,
            });
            log.error('DEBUGINFO 1.2', {
                function: vault.checkPolicies,
                vault,
            });
            return vault.checkPolicies(requestContextParams, authInfo.getArn(),
                log, (err, authorizationResults) => {
                    log.error('DEBUGINFO 2', {
                        err,
                    ...
                    if (requestContextParams.parameterize.specificResource.length
                        ...
                        log.error('vault did not return correct number of ' +
                            ...
                            authorizationResultsLength:
                                authorizationResults.length,
                            objectsLength:
                                requestContextParams.parameterize.specificResource.length,
                        });
                    ...
                    for (let i = 0; i < authorizationResults.length; i++) {
                        ...
                        const entry = {
                            key,
                            versionId: result.versionId || objects.find(o => o.key === key).versionId,
                        };
                        if (result.isAllowed) {
                            inPlay.push(entry);
                        } else {
